...
BOOL CLab3Dlg::OnInitDialog()
{
	...
	ti.pszText = _T("Вхідні параметри");
	ti.mask = TCIF_TEXT;
	m_selection.InsertItem(0, &ti);

	ti.pszText = _T("Таблиця коєфіцієнтів");
	ti.mask = TCIF_TEXT;
	m_selection.InsertItem(1, &ti);

    ti.pszText = _T("Матриці обчислень");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(2, &ti);

    ti.pszText = _T("Перевірка 1");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(3, &ti);

    ti.pszText = _T("Матриці обчислень 2");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(4, &ti);

    ti.pszText = _T("Перевірка 2");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(5, &ti);

    ti.pszText = _T("Графіки");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(6, &ti);
...
void CLab3Dlg::OnTcnSelchangeTab(NMHDR* pNMHDR, LRESULT* pResult)
{
    RECT rect;
    RECT itemRect;
	...
	if (pNMHDR->hwndFrom == m_selection.m_hWnd)
	{
        ...
        else if (selected == 4)
        {
            ...
            // create fifth tab
            //
            m_CurrentTab = new CFifthTab(&m_solver);
        ...
        else if (selected == 5)
        {
            ...
            // create six tab
            //
            m_CurrentTab = new CSixTab(&m_solver);
        ...
        else if (selected == 6)
        {
            ...
            // create seven tab
            //
            m_CurrentTab = new CSevenTab(&m_solver);
