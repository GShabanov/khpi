...
CFifthTab::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd)
{
    ...
    const CMatrix<double>& aMatrix = m_solver->getAMatrix2();
    ...
    // B matrix
    //
    const CMatrix<double>& bMatrix = m_solver->getBMatrix2();
    ...
    // X matrix
    //
    const CMatrix<double>& xMatrix = m_solver->getXMatrix2();
...
CFifthTab::OnPaint()
{
    ...
    CDC* dc;
    CString         text = _T("Матриці обчислень для кубічного рівняння");
