...
BOOL CLab2Dlg::OnInitDialog()
{
    ...
    ti.pszText = _T("Перевірка обчислень");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(2, &ti);

    ti.pszText = _T("Оцінка коефіцієнтів");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(3, &ti);

    ti.pszText = _T("Перевірка значимості вцілому");
    ti.mask = TCIF_TEXT;
    m_selection.InsertItem(4, &ti);

    ti.pszText = _T("Графік");
    ti.mask = TCIF_TEXT;
...
void CLab2Dlg::OnTcnSelchangeTab(NMHDR* pNMHDR, LRESULT* pResult)
{
    RECT rect;
    RECT itemRect;
