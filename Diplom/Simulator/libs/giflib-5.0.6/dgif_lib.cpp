...
DGifGetScreenDesc(GifFileType *GifFile)
{
    ...
    {    /* Do we have global color map? */
        ...
        for (unsigned int i = 0; i < GifFile->SColorMap->ColorCount; i++)
        {
            if (READ(GifFile, Buf, 3) != 3)
            {
                GifFreeMapObject(GifFile->SColorMap);
...
DGifGetImageDesc(GifFileType *GifFile)
{
    ...
    if (Buf[0] & 0x80) 
    {
        ...
        for (i = 0; i < GifFile->Image.ColorMap->ColorCount; i++) // BUGGGGGGG !!!!!!!
        {
            if (READ(GifFile, Buf, 3) != 3)
            {
                GifFreeMapObject(GifFile->Image.ColorMap);
...
******************************************************************************/
static int
DGifDecompressLine(GifFileType *GifFile, GifPixelType *Line, unsigned int LineLen)
{
    unsigned int i = 0, j = 0;
    int EOFCode, CrntPrefix, LastCode, StackPtr;
    int CrntCode, ClearCode;
    ...
    do {
        ...
        switch (RecordType) {
	      ...
	      if (sp->ImageDesc.Interlace)
          {
              ...
              * offsets and jumps...
              */
              GifWord InterlacedOffset[] = { 0, 4, 2, 1 };
              GifWord InterlacedJumps[] = { 8, 8, 4, 2 };
              /* Need to perform 4 passes on the image */
              for (GifWord i = 0; i < 4; i++)
              {
                  for (GifWord j = InterlacedOffset[i]; 
